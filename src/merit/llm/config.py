...
from merit.llm.defaults import FileAccessLevel, Tool, MAX_AGENT_TURNS
...
class LLMConfig(BaseSettings):
    ...
    model_vendor: Literal["anthropic", "openai"] = Field(
        description="The model vendor (anthropic or openai)"
    )
    ...
    region: str | None = Field(
        default=None,
        validation_alias="CLOUD_ML_REGION",
        description="Cloud region (for GCP Vertex AI)"
    )
    project_id: str | None = Field(
        default=None,
        validation_alias="GOOGLE_CLOUD_PROJECT",
        description="Cloud project ID (for GCP Vertex AI)"
    )
