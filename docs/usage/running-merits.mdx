...
The database location is determined by walking up from the current directory to find the project root (identified by `.git`, `pyproject.toml`, or `setup.py`).
...
Without `--yes`, you'll be prompted for confirmation.
...
- `merit_runs` - Test run sessions
- `test_executions` - Individual test results
- `assertion_results` - Assertion outcomes
